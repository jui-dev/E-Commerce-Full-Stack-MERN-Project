...
const Categories = () => {
  const { CategoryList } = ProductStore(); // using the "CategoryList" property from the "ProductStore()"
          ...
          <div className="row">
            ...
            {CategoryList.map((item, i) => {
              return (
                ...
                >
                  <Link
                    to={`/by-category/${item["_id"]}`} // /by-category = routing.
                    className="card h-100 rounded-3 bg-white"
                  >
                    <div className="card-body">
                      {/* ------ getting category's image------- */}
