...
const ProductByBrand = () => {
  const { ListByBrandRequest } = ProductStore(); // using the "ListByBrandRequest" property from the "ProductStore()" and call it inside the useEffect Hook.
  const { id } = useParams(); // we will send "id" at the url parameter.

  // we will call API inside the UseEffect.
  useEffect(() => {
    (async () => {
      await ListByBrandRequest(id); // calling the "ListByBrandRequest(id)" method inside the useEffect Hook.
    })();
